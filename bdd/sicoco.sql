...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_GENERA_PAGOS` (IN `_IDARRIENDO` INT)   BEGIN
    ...
    IF _FECHA_FIN IS NOT NULL THEN
        -- Contrato Temporal: se genera un solo pago con el monto prorrateado total
        SET CADENA = CONCAT('TEMP (', DATE_FORMAT(_FECHA, '%d/%m'), ' al ', DATE_FORMAT(_FECHA_FIN, '%d/%m'), ')');
        INSERT INTO pagos (IDARRIENDO, PERIODO, MONTO) VALUES (_IDARRIENDO, CADENA, _MONTO);
    ELSE
        -- Contrato Normal: flujo mensual estándar
        SET I = 0;
        WHILE I < _TIEMPO DO
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_INSERT_CLIENTE` (
    ...
    IN `_CONTACTOS` VARCHAR(20), 
    IN `_DIRECCION` VARCHAR(70),
    IN `_REF_NOMBRE` VARCHAR(150),
    IN `_REF_PARENTESCO` VARCHAR(50),
    IN `_REF_CELULAR` VARCHAR(20),
    IN `_REF_DIRECCION` VARCHAR(150),
    IN `_REF_COORDENADAS` VARCHAR(100)
)   BEGIN
DECLARE OP VARCHAR(1);
SET OP='1';
INSERT INTO clientes (
    NOMBRE_COMPLETO, CEDULA, CONTACTOS, DIRECCION,
    REF_NOMBRE, REF_PARENTESCO, REF_CELULAR, REF_DIRECCION, REF_COORDENADAS
) VALUES (
    UPPER(_NOMBRE), _CEDULA, _CONTACTOS, UPPER(_DIRECCION),
    UPPER(_REF_NOMBRE), UPPER(_REF_PARENTESCO), _REF_CELULAR, UPPER(_REF_DIRECCION), _REF_COORDENADAS
);
SELECT OP;
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_MOD_ARRENDAMIENTO_UNIFICADO` (
    IN `_IDARRIENDO` INT, 
    IN `_CEDULA` VARCHAR(15), 
    ...
    IN `_DIRECCION` VARCHAR(70), 
    IN `_IDCATALOGO` INT, 
    IN `_ACTIVIDAD` VARCHAR(100), 
    IN `_RAZONSOCIAL` VARCHAR(100), 
    IN `_CONTRATO` VARCHAR(30), 
    IN `_FECHA_SUSCRIPCION` VARCHAR(10), 
    IN `_FECHAINICIO` VARCHAR(10), 
    IN `_FECHA_FIN` VARCHAR(10),
    IN `_TIEMPOCONTRATO` INT
)   BEGIN
    DECLARE _IDCLIENTE INT;
    DECLARE _ALQUILER DECIMAL(10,2);
    DECLARE _NOMBRE_COMPLETO VARCHAR(150);
    DECLARE _DIAS_CONTRATO INT;
    DECLARE _MONTO_FINAL DECIMAL(10,2);
    ...
    UPDATE arriendos SET 
        ...
        FECHA_FIN = _FECHA_FIN, TIEMPOCONTRATO = _TIEMPOCONTRATO
    WHERE IDARRIENDO = _IDARRIENDO;
    ...
    -- Determinar el monto final a cobrar
    IF _FECHA_FIN IS NOT NULL AND _FECHA_FIN != '' THEN
        -- Contrato Temporal: Calcular cantidad de días exactos y el monto prorrateado
        SET _DIAS_CONTRATO = DATEDIFF(STR_TO_DATE(_FECHA_FIN, '%Y-%m-%d'), STR_TO_DATE(_FECHAINICIO, '%Y-%m-%d')) + 1;
        SET _MONTO_FINAL = ROUND((_ALQUILER / DAY(LAST_DAY(STR_TO_DATE(_FECHAINICIO, '%Y-%m-%d')))) * _DIAS_CONTRATO, 2);
        
        UPDATE arriendos SET MONTO = _MONTO_FINAL, TIEMPOCONTRATO = _DIAS_CONTRATO WHERE IDARRIENDO = _IDARRIENDO;
    ELSE
        -- Contrato Regular: Se calcula por los periodos mensuales estándares del catálogo
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_MOD_CLIENTE` (
    IN `_IDCLIENTE` INT, 
    IN `_NOMBRE` VARCHAR(150), 
    IN `_CEDULA` VARCHAR(15), 
    IN `_CONTACTOS` VARCHAR(20), 
...
UPDATE clientes SET
    ...
    REF_NOMBRE=UPPER(_REF_NOMBRE),
    REF_PARENTESCO=UPPER(_REF_PARENTESCO),
    REF_CELULAR=_REF_CELULAR,
    REF_DIRECCION=UPPER(_REF_DIRECCION),
    REF_COORDENADAS=_REF_COORDENADAS
WHERE IDCLIENTE=_IDCLIENTE;
SELECT OP;
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_NUEVO_ARRENDAMIENTO` (
    IN `_IDUSUARIO` INT, 
    IN `_CEDULA` VARCHAR(15), 
    IN `_NOMBRES` VARCHAR(50), 
    IN `_PATERNO` VARCHAR(50), 
    ...
    INSERT INTO arriendos (IDUSUARIO, IDCLIENTE, ACTIVIDAD, RAZONSOCIAL, CONTRATO, FECHA_SUSCRIPCION, FECHA_INICIO, FECHA_FIN, TIEMPOCONTRATO) 
    VALUES (_IDUSUARIO, _IDCLIENTE, UPPER(_ACTIVIDAD), UPPER(_RAZONSOCIAL), UPPER(_CONTRATO), _FECHA_SUSCRIPCION, _FECHAINICIO, _FECHA_FIN, _TIEMPOCONTRATO);
...
CREATE TABLE `clientes` (
  ...
  `REF_NOMBRE` varchar(150) DEFAULT NULL,
  `REF_PARENTESCO` varchar(50) DEFAULT NULL,
  `REF_CELULAR` varchar(20) DEFAULT NULL,
  `REF_DIRECCION` varchar(150) DEFAULT NULL,
  `REF_COORDENADAS` varchar(100) DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_contratos`  AS SELECT `a`.`IDARRIENDO` AS `IDARRIENDO`, `a`.`IDUSUARIO` AS `IDUSUARIO`, `a`.`IDCLIENTE` AS `IDCLIENTE`, `a`.`ACTIVIDAD` AS `ACTIVIDAD`, `a`.`RAZONSOCIAL` AS `RAZONSOCIAL`, `a`.`CONTRATO` AS `CONTRATO`, `a`.`FECHA_INICIO` AS `FECHA_INICIO`, `a`.`FECHA_FIN` AS `FECHA_FIN`, `a`.`TIEMPOCONTRATO` AS `TIEMPOCONTRATO`, `a`.`MONTO` AS `MONTO`, `a`.`OBSERVACIONES` AS `OBSERVACIONES`, `a`.`VIGENTE` AS `VIGENTE`, `a`.`FECHA_REGISTRO` AS `FECHA_REGISTRO`, concat(`c`.`CEDULA`,' - ',`c`.`NOMBRE_COMPLETO`) AS `REPRESENTANTE` FROM (`arriendos` `a` join `clientes` `c`) WHERE `a`.`IDCLIENTE` = `c`.`IDCLIENTE` ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_resumen_gral_contrato`  AS SELECT `v_contratos`.`IDARRIENDO` AS `IDARRIENDO`, concat(`v_contratos`.`REPRESENTANTE`,' - ACTIVIDAD: ',`v_contratos`.`ACTIVIDAD`,' - RAZON SOCIAL: ',`v_contratos`.`RAZONSOCIAL`) AS `GENERAL`, IF(`v_contratos`.`FECHA_FIN` IS NULL, concat('CONTRATO: ',`v_contratos`.`CONTRATO`,' - INICIO: ',`v_contratos`.`FECHA_INICIO`,' - TIEMPO CONTRATO: ',`v_contratos`.`TIEMPOCONTRATO`,' [MESES]'), concat('CONTRATO: ',`v_contratos`.`CONTRATO`,' - INICIO: ',`v_contratos`.`FECHA_INICIO`,' - FIN: ',`v_contratos`.`FECHA_FIN`,' - TIEMPO CONTRATO: ',`v_contratos`.`TIEMPOCONTRATO`,' [DÍAS]')) AS `REFERENCIAL`, IF(`v_contratos`.`FECHA_FIN` IS NULL, concat('IMPORTE MENSUAL BS: ',`v_contratos`.`MONTO`), concat('IMPORTE TOTAL TEMPORAL BS: ',`v_contratos`.`MONTO`)) AS `ESPECIFICO`, `v_contratos`.`VIGENTE` AS `VIGENTE` FROM `v_contratos` ;
