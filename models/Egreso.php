...
class Egreso {
    ...
    public function listar_egresos($inicio, $fin, $tipo) {
        $sql_propuesta = "SELECT 'Garantía de Propuesta' AS TIPO, IDPROPUESTA AS NRO, NOMBRE_POSTULANTE AS CLIENTE, CI_POSTULANTE AS CI, MONTO, FECHA_DEVOLUCION AS FECHA FROM propuestas WHERE ESTADO = 'DEVUELTA' AND DATE(FECHA_DEVOLUCION) BETWEEN ? AND ?";
